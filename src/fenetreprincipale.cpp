            ...
            widgetOnglet->addTab(ongletArbre, QIcon(":/images/arbre/arbre.ico"), "Arbre");
                    ...
                    layoutArbre->setContentsMargins(10,10,10,10);
                        ...
                        outilsArbre->addSeparator();
                            ...
                            zoomResetAction = outilsArbre->addAction(QIcon(":/images/arbre/zoomReset.ico"), "Dézoom");
                    ...
                    layoutArbre->addWidget(arbreGraphicsView);
                        ...
                        // Zone de vérification de l'arbre
                        verifArbre = new QWidget(widgetOnglet);

                            // Layout de la zone de vérification de l'arbre
                            verifArbreLayout = new QGridLayout(verifArbre);

                                // Titre de la partie de vérification de l'arbre
                                titreVerifArbre = new QLabel("Vérification / comparaison avec un dictionnaire", verifArbre);

                            verifArbreLayout->addWidget(titreVerifArbre, 0, 0, 1, 3, Qt::AlignCenter);

                                // Chemin du dictionnaire avec lequel comparé l'arbre
                                cheminVerifArbre = new QLineEdit(verifArbre);
                                QObject::connect(cheminVerifArbre, SIGNAL(textChanged(QString)), this, SLOT(coulInitCheminVerifTexte()));

                            verifArbreLayout->addWidget(cheminVerifArbre, 1, 0, 1, 1);

                                // Bouton de recherche de fichier
                                parcourirVerifArbre = new QPushButton("...", verifArbre);
                                parcourirVerifArbre->setFixedWidth(40);
                                QObject::connect(parcourirVerifArbre, SIGNAL(clicked(bool)), this, SLOT(ouvrirVerifArbre()));

                            verifArbreLayout->addWidget(parcourirVerifArbre, 1, 1, 1, 1);

                                // Bouton de lancement de la vérification
                                lancerVerifArbre = new QPushButton("Comparer avec mon arbre", verifArbre);
                                QObject::connect(lancerVerifArbre, SIGNAL(clicked(bool)), this, SLOT(veriferArbre()));

                            verifArbreLayout->addWidget(lancerVerifArbre, 1, 2, 1, 1);

                    layoutArbre->addWidget(verifArbre);
...
// Colorie en rouge le chemin indiquant le texte à comparer avec l'arbre
void FenetrePrincipale::coulErreurCheminVerifTexte()
{
    cheminVerifArbre->setStyleSheet("border : 1px solid red; padding: 1px;");
}
...
// Rétablit la couleur d'origine du chemin indiquant le texte à comparer avec l'arbre
void FenetrePrincipale::coulInitCheminVerifTexte()
{
    cheminVerifArbre->setStyleSheet("");
}
...
// Sélectionne le texte avec lequel l'arbre sera comparé
void FenetrePrincipale::ouvrirVerifArbre()
{
    cheminVerifArbre->setText(ouvrirFichierChemin());
}

// Lance la vérification de l'arbre en le comparant avec celui généré avec le texte indiqué
void FenetrePrincipale::veriferArbre()
{
    // On recalcule le codage de l'arbre courant
    huffman->calculCodage(huffman->getArbreHuff());

    // On vérifie si le chemin du texte avec lequel vérifié l'arbre n'est pas vide
    if(cheminVerifArbre->text().isEmpty())
    {
        // S'il l'est, on affiche une fenetre de dialogue d'erreur
        QMessageBox::warning(this, "Pas de texte d'indiqué !", "Il est impossible de vérifier l'arbre sans indiquer un texte !");
        // Et on change la couleur de la ligne de texte contenant le chemin
        coulErreurCheminVerifTexte();
    }
    else
    {
        // Sinon on lance la vérification de l'arbre avec ce texte
        try
        {
            huffman->verifArbre(ouvrirFichierTexte(cheminVerifArbre->text()));

            // S'il n'y a pas eu d'exception, c'est que la vérification est positive !
            QMessageBox::information(this, "Arbre correct !", "L'arbre comprend bien le même codage que le texte de cryptage indiqué.");
        }
        catch(QString msgErreurVerif)
        {
            QMessageBox::critical(this, "Arbre incorrect !", msgErreurVerif);
        }
    }
}
