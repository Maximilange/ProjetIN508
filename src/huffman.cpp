...
// Vérifie que l'arbre binaire inclut bien celui du texe passé en argument
void Huffman::verifArbre(QString texte)
{
    // On procède à l'algorithme de Huffman avec le texte à comparé avec l'arbre
    Huffman huffmanTexte(texte);

    Codage* codage;
    int erreur = 0;
    // On vérifie un à un si les caractères du texte sont codés de la même manière dans l'arbre
    CNoeud<Codage>* ptrListeCodage = huffmanTexte.getCodageHuff()->getTete();
    while(ptrListeCodage !=  0 && erreur != 1)
    {
        // On vérifie tout d'abord que le caractère courant est présent dans la table de codage de l'arbre
        codage = codageHuff->rechercheObjPtr(ptrListeCodage->getObjet());

        // Si le caractère courant n'est pas présent, alors l'arbre n'est pas valide avec ce texte de cryptage
        if(codage == 0)
        {
            throw QString("Le caractère '" + QString(ptrListeCodage->getObjet().getCarac()) + "' de ce texte de cryptage n'est pas présent dans l'arbre !");
        }
        // Sinon on va également vérifier si le codage du caractère est le même
        else
        {
            // Si ils n'ont pas le même codage, alors l'arbre n'est pas valide avec ce texte de cryptage
            if(codage->getCodeBin() != ptrListeCodage->getObjet().getCodeBin())
            {
                throw QString("Le caractère '" + QString(codage->getCarac()) + "' n'a pas le même codage dans cet arbre qu'avec ce texte de cryptage");
            }
            else
            {
                // Sinon on continue notre vérification
                ptrListeCodage = ptrListeCodage->getSuivant();
            }
        }
    }
}
